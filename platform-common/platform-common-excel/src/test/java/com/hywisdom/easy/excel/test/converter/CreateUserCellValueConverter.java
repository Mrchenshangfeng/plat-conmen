package com.hywisdom.easy.excel.test.converter;

import com.hywisdom.easy.excel.config.FieldValue;
import com.hywisdom.easy.excel.exception.ExcelException;
import com.hywisdom.easy.excel.parsing.CellValueConverter;
...
public class CreateUserCellValueConverter implements CellValueConverter {
	...
	public Object convert(Object bean, Object value, FieldValue fieldValue, Type type, int rowNum)
			throws Exception {
		System.out.println("CreateUserCellValueConverter:20行输出：" + fieldValue.getOtherConfig());
		//如果是导入
		if (type == Type.IMPORT) {
	...
	private boolean queryForDb(String createUser) {
		if (createUser.startsWith("王五")) {
			System.out.println("数据库有查询到王五,发生在com.hywisdom.easy.excel.test.converter.CreateUserCellValueConverter");
			return true;
		}
		return false;
	}
