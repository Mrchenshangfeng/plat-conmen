...
import com.hywisdom.easy.util.DateUtil;
import com.hywisdom.easy.util.ReflectUtil;
import com.hywisdom.easy.util.SpringUtil;
...
import com.hywisdom.easy.excel.util.ExcelUtil;
		...
		if(value !=null){
			...
			if(convName==null){
				...
				if (StringUtils.isNotBlank(pattern)) {
					...
					return resolverExpression(value.toString().trim(), format, type,fieldValue,rowNum);
				} else if (decimalFormat!=null) {
					if (Type.IMPORT == type) {
						if(value instanceof String){
							return decimalFormat.parse(value.toString().trim());
						}
