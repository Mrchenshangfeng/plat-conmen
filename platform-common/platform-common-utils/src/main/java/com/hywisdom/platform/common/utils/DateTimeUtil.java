...
public class DateTimeUtil {
    ...
    public static Date setDayPlus(String date, int dayAmount) {
        ...
        DateTime dateTime = new DateTime(date);
        if (dayAmount >= 29) {
            Date[] dates = DateTimeUtil.computeMonthFirstAndLastDay(DateTimeUtil.getDate(date), 0);
            String dateStr = DateTimeUtil.getDateToString(dates[1]);
    ...
    public static List<Date> computeDateRangeDate(Date from, Date to) {
        ...
        from = DateTimeUtil.getDateFormat(from, DATE_DEFAULT_FORMAT);
        to = DateTimeUtil.getDateFormat(to, DATE_DEFAULT_FORMAT);
    ...
    public static Date[] computeMonthFirstAndLastDay(Date sourceDate, int monthInterval) {
        ...
        destDates[0] = DateTimeUtil.getDate(DATE_FORMATTER.get().format(cal.getTime()));
        if (monthInterval < 0) {
            cal.setTime(sourceDate);
            if (monthInterval < -1) {
        ...
        destDates[1] = DateTimeUtil.getDate(DATE_FORMATTER.get().format(cal.getTime()));
    ...
    public static String getAfterDay(Date date) {
        ...
        return DateTimeUtil.getDateToString(date);
    }
    ...
    public static Date getRedSpotDate() {
        Date redSpotDate = DateTimeUtil.getDate("2018-2-21");
        return redSpotDate;
    }
...
//        int totalMonth = diff.get(0)*12 + diff.get(1) + (diff.get(2) > 0?1:0);
        ...
        if (DateTimeUtil.getDay(start) <= dayOfMonth) {
            Date[] dates = DateTimeUtil.computeMonthFirstAndLastDay(DateTimeUtil.getDate(start), 0);
            String dateStr = DateTimeUtil.getDateToString(dates[1]);
            String dayStr = dateStr.substring(dateStr.length() - 2);
            Integer day = Integer.valueOf(dayStr);
            if (dayOfMonth <= day) {
                String computeDate = dateStr.substring(0, dateStr.lastIndexOf("-") + 1);
                Date targetDate = DateTimeUtil.getDate(computeDate + dayOfMonth);
                dateList.add(targetDate);
            }
        }
        //如果目标天大于结束日期的天那么只计算到结束日期的上个月
        if (DateTimeUtil.getDay(end) < dayOfMonth) {
            totalMonth--;
        }
        String sourceDate = DateTimeUtil.getDateToString(start);
        for (int i = 0; i < totalMonth; i++) {
            sourceDate = DateTimeUtil.getDateToString(DateTimeUtil.addMonth(sourceDate, 1));
            String computeDate = sourceDate.substring(0, sourceDate.lastIndexOf("-") + 1);
            Date[] dates = DateTimeUtil.computeMonthFirstAndLastDay(DateTimeUtil.getDate(sourceDate), 0);
            String dateStr = DateTimeUtil.getDateToString(dates[1]);
            String dayStr = dateStr.substring(dateStr.length() - 2);
            Integer day = Integer.valueOf(dayStr);
            if (dayOfMonth <= day) {
...
//        int totalMonth = diff.get(0)*12 + diff.get(1) + (diff.get(2) > 0?1:0);
        ...
        if (DateTimeUtil.getDay(start) <= dayOfMonth) {
            ...
            if (dayOfMonth <= day) {
                ...
                Date targetDate = DateTimeUtil.getDate(computeDate);
                dateList.add(targetDate);
            }
        }
        //如果目标天大于结束日期的天那么只计算到结束日期的上个月
